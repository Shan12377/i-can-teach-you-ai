...
### Rule 6b: Portfolio Asset Boundary
Raw files belonging to Hunter's Holistic Health or any other project (working screenshots, client content, private dashboards, exports, notes, and source media) never enter this repo. A reviewed, public-safe portfolio derivative may enter `public/proof/` only when Dr. Hunter approved the use, it contains no patient-identifiable information, personal email information, private notes, or clinical inquiry details, and its provenance is recorded in the private redesign memory. If an unreviewed foreign asset appears in `git status`, say so and do not stage it.
...
9. **Homepage layout and motion** are specified in `docs/DESIGN-SPEC.md`. The founder-led Living Build Lab direction replaces the older Rx terminal prototype. Follow the current spec exactly.
...
11. **One primary CTA in the hero.** The homepage primary action opens proof of work. The organization service action is secondary. Product and waitlist actions appear later in the page.
